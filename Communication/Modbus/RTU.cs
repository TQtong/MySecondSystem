...
namespace Communication.Modbus
{
    ...
    public class RTU
    {
        public Action<int, List<byte>> ResponseData;//返回数据(报文体)

        private static RTU _instance;//创建单例防止串口多次初始化报错

        private static SerialInfo _serialInfo;

        SerialPort _serialPort;//报文
        bool _isBusing = false;

        int _currentSlave;//从站地址
        int _funcCode;//功能码
        int _wordLen;//字节长度
        int _startAddr;//起始地址
        ...
        byte[] _byteBuffer = new byte[512];//字节缓冲区，用于储存报文数据
        ...
        private void _serialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (_byteBuffer[0] == (byte)_currentSlave && _byteBuffer[1] == _funcCode && _receiveByteCount >= _wordLen + 5)//加5是加上报文的报头和报尾(crc校验,两个长度字节)
            {
                // 检查crc
                // ...........
                // 返回数据
        ...
        public async Task<bool> Send(int slaveAddr, byte funcCode, int startAddr, int len)
        {
            ...
            byte[] crc = Crc16(sendBuffer.ToArray(), 6);//crc校验
            sendBuffer.AddRange(crc);

            try
            {
                while (_isBusing) { } //防止并发报错
